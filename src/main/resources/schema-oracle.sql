...
CREATE TABLE CITIES(
  ...
  COUNTRY_ID NUMBER (4) NOT NULL REFERENCES COUNTRIES(COUNTRY_ID),
  PRIMARY KEY (CITY_ID)
 );
...
--     REFERENCES COUNTRIES(COUNTRY_ID);
 ...
 CREATE TABLE ADDRESSES(
  ...
  CITY_ID NUMBER (4) NOT NULL REFERENCES CITIES (CITY_ID),
...
CREATE TABLE COTTAGES_FACILITIES(
    COTTAGE_ID NUMBER (4) CONSTRAINT COTTAGE_FACILITY_FK REFERENCES COTTAGES (COTTAGE_ID),
    FACILITY_ID NUMBER (4) CONSTRAINT FACILITY_FK REFERENCES FACILITIES (FACILITY_ID),
...
CREATE TABLE COTTAGES_EQUIPMENTS(
    COTTAGE_ID NUMBER (4) CONSTRAINT COTTAGE_EQUIPMENT_FK REFERENCES COTTAGES (COTTAGE_ID),
    EQUIPMENT_ID NUMBER (4) CONSTRAINT EQUIPMENT_FK REFERENCES EQUIPMENTS (EQUIPMENT_ID),
    PRIMARY KEY (COTTAGE_ID, EQUIPMENT_ID)
);

CREATE TABLE RESTRICTIONS(
    RESTRICTION_ID NUMBER (4) NOT NULL,
    NAME VARCHAR2(40) NOT NULL,
    DESCRIPTION VARCHAR2(100),
...
CREATE TABLE COTTAGES_RESTRICTIONS(
    COTTAGE_ID NUMBER (4)  CONSTRAINT COTTAGE_RESTRICTION_FK REFERENCES COTTAGES (COTTAGE_ID),
    RESTRICTION_ID NUMBER (4) CONSTRAINT RESTRICTION_FK REFERENCES RESTRICTIONS (RESTRICTION_ID),
...
CREATE TABLE REQUESTS(
    ...
    COTTAGE_ID NUMBER (4) NOT NULL CONSTRAINT REQUEST_COTTAGE_FK REFERENCES COTTAGES (COTTAGE_ID) ON DELETE CASCADE,
    CHECKIN_DATE DATE NOT NULL,
    CHECKOUT_DATE DATE NOT NULL,
    TOTAL_PRICE DECIMAL(7, 2) NOT NULL,
    CUSTOMER_ID NUMBER (4) NOT NULL CONSTRAINT REQUEST_CUSTOMER_FK REFERENCES USERS (USER_ID),
    PRIMARY KEY (REQUEST_ID)
);

CREATE TABLE REQUEST_APPROVALS (
  REQUEST_APPROVAL_ID NUMBER (4) NOT NULL,
  DATE_CREATED DATE NOT NULL,
  IS_APPROVED NUMBER (1) NOT NULL,
  REQUEST_ID NUMBER (4) NOT NULL CONSTRAINT REQ_APPROVAL_REQ_FK REFERENCES REQUESTS (REQUEST_ID) ON DELETE CASCADE,
  PRIMARY KEY (REQUEST_APPROVAL_ID)
);

CREATE TABLE REVIEWS(
    REVIEW_ID NUMBER (4) NOT NULL,
    COTTAGE_ID NUMBER (4) NOT NULL CONSTRAINT REVIEW_COTTAGE_FK REFERENCES COTTAGES (COTTAGE_ID) ON DELETE CASCADE,
    AUTHOR_ID NUMBER (4) NOT NULL CONSTRAINT AUTHOR_COTTAGE_FK REFERENCES USERS (USER_ID),
    TEXT VARCHAR2(1020),
